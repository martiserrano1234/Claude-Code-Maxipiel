...
async function main() {
  ...
  // Config fija de agentes conocidos
  const agentConfig: Record<string, { sprite: string; position: string }> = {
    sofia: { sprite: 'nova', position: workAnchors[1] ?? 'desk_corner_right_1_0' },
    claude: { sprite: 'morty', position: workAnchors[0] ?? 'desk_corner_left_0_0' },
  };
  ...
  const spriteSheets: Record<string, SpriteSheetConfig> = {
    ...Object.fromEntries(serverAgents.map((a: any, i: number) => {
      const spriteName = agentConfig[a.agent]?.sprite ?? availableSprites[i % availableSprites.length];
      return [a.agent, charSprites(spriteName)];
    }))
  };
  ...
  const mv = new Miniverse({
    ...
    citizens: [
      ...serverAgents.map((a: any, i: number) => ({
        ...
        name: a.name || a.agent,
        sprite: a.agent,
        position: agentConfig[a.agent]?.position ?? workAnchors[i] ?? sceneData?.wanderPoints?.[i]?.name ?? 'wander_' + i,
      }))
    ],
    scale: 2,
